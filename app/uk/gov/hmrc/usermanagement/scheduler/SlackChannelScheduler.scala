...
, ExecutionContext
  ...
  def manualSync(testMode: Boolean)(using HeaderCarrier): Future[Unit] =
    slackChannelLock
      .withLock:
        logger.info("Slack channel sync has been manually triggered")
        for
          allTeams       <- umpConnector.getAllTeams()
          githubTeams    <- teamsAndRepositoriesConnector.allTeams()
          teamsToProcess =  allTeams
        ...
        yield ()
      .map:
        _.getOrElse(logger.info(s"The Slack channel sync process is locked for ${slackChannelLock.lockId}"))
